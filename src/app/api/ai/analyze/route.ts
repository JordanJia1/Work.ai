...
export async function POST(request: NextRequest) {
  ...
  const userPrompt = `Today is ${today}. Analyze each task for realistic effort and priority for a weekly execution plan.\n\nRules:\n- Infer complexity and business impact from title/details/deadline only.\n- estimatedHours: numeric hours (0.5 to 80)\n- urgencyScore: integer 0-100 based on exact deadline proximity\n- priorityScore: integer 0-100 combining urgency, inferred impact, inferred complexity, and dependency/risk clues\n- priorityLabel: one of Critical|High|Medium|Low\n- analysisReason: <= 180 chars concise rationale and must include deadline-risk reasoning.\n- isSplittable: boolean. Decide this from task semantics and execution constraints (not keyword matching). Set false only if task should be done in one continuous session.\n- notBeforeISO: ISO datetime string or null. Use when details imply work cannot start before a specific date/time (e.g., opens on March 1).\n\nScheduling-feasibility policy:\n- Do not assign very high priority without considering estimatedHours vs time remaining before deadline.\n- For large tasks, prefer early starter progress (at least one early execution block) to reduce end-loaded deadline risk.\n- If a later-due task is scored above a near-due task, analysisReason must explicitly justify why.\n\nDeadline-risk policy:\n- If a task is due within 72 hours, prioritize deadline risk over long-term impact.\n- If two tasks differ by >=24 hours in due date and one is due within 72 hours, boost the earlier task's urgency/priority.\n- Do not let later-due tasks starve earlier-due tasks unless the later task has truly critical failure impact.\n\nReturn JSON only as:\n{\"analyses\":[{\"id\":\"...\",\"estimatedHours\":0,\"urgencyScore\":0,\"priorityScore\":0,\"priorityLabel\":\"Medium\",\"analysisReason\":\"...\",\"isSplittable\":true,\"notBeforeISO\":null}]}\n\nTasks:\n${JSON.stringify(tasks)}`;
